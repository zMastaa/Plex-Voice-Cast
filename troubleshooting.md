## Troubleshooting Plex Voice Cast

Whenever posting an issue include as much of the following info as you can: 

* Any errors related to Plex Voice Cast or Plex in your logs along with debug info (see below on how to enable debug)
...
* If using the `plex_voice_cast.command` service in HA's Developer Tools is working
...
Add the following to your configuration.yaml file, restart, ask Plex Voice Cast to do something, go to your logs and view full logs.

```yaml
logger:
  default: warning
  logs:
    custom_components.plex_voice_cast: debug
```
